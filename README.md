...
项目已配置 GitHub Actions 自动打包，前往 [Releases 页面](https://github.com/sxlixiaoyang/youtube-live-relay/releases) 下载对应平台版本：

| 平台 | 架构 | 归档格式 | 文件名 |
|------|------|---------|--------|
| Windows | x86_64 | zip | `youtube-live-relay-windows-amd64.zip` |
| macOS  | Intel (x86_64) | tar.gz | `youtube-live-relay-macos-x86_64.tar.gz` |
| macOS  | Apple Silicon (arm64) | tar.gz | `youtube-live-relay-macos-arm64.tar.gz` |
| Linux  | x86_64 | tar.gz | `youtube-live-relay-linux-x86_64.tar.gz` |

打包内容已包含：
- 主程序（PyInstaller 打包，无需 Python 环境）
- FFmpeg 二进制
- mediamtx 二进制
- Chrome 扩展（`chrome-extension/`）
- `config.yaml.example` 模板
- `启动转播.bat` / `启动转播.sh` 启动脚本

### 使用步骤

1. 解压归档
2. 复制 `config.yaml.example` 为 `config.yaml`，填入代理和直播地址
3. 双击 `启动转播.bat`（Windows）或运行 `./启动转播.sh`（macOS / Linux）
4. 浏览器打开 http://localhost:5000 进入 Web 面板

### macOS 首次运行提示

未签名的二进制首次运行会被 Gatekeeper 拦截，命令行执行：

```bash
xattr -cr youtube-relay/youtube-relay
xattr -cr youtube-relay/mediamtx/mediamtx
xattr -cr youtube-relay/ffmpeg/bin/ffmpeg
```
